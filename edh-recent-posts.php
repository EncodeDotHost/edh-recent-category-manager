...
function edh_rp_on_option_updated( $option, $old_value, $new_value ) {
    ...
    if ( in_array( $option, array( 'edh_recent_posts_category', 'edh_recent_posts_count' ), true ) ) {
        raac_update_recent_articles_category();
    }
}
...
add_action( 'transition_post_status', 'raac_trigger_on_status_change', 10, 3 );
function raac_trigger_on_status_change( $new_status, $old_status, $post ) {
    if ( 'post' === $post->post_type ) {
        raac_update_recent_articles_category();
    }
}

add_action( 'deleted_post', 'raac_trigger_on_deletion' );
function raac_trigger_on_deletion( $post_id ) {
    if ( 'post' === get_post_type( $post_id ) ) {
        raac_update_recent_articles_category();
    }
}

register_activation_hook( __FILE__, 'raac_activate_plugin' );
function raac_activate_plugin() {
    raac_update_recent_articles_category();
}
...
function raac_update_recent_articles_category() {
